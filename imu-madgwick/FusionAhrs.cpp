...
#define Q fusionAhrs->quaternion // define shorthand labels for more readable code
#define A fusionAhrs->linearAcceleration
    ...
    const FusionVector3 earthAcceleration = {
        2.0f * ((qwqw - 0.5f + Q.x * Q.x) * A.x + (qxqy - qwqz) * A.y + (qxqz + qwqy) * A.z), // x
        2.0f * ((qxqy + qwqz) * A.x + (qwqw - 0.5f + Q.y * Q.y) * A.y + (qyqz - qwqx) * A.z), // y
        2.0f * ((qxqz - qwqy) * A.x + (qyqz + qwqx) * A.y + (qwqw - 0.5f + Q.z * Q.z) * A.z), // z
